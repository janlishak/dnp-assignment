...
namespace AdultsWebAPI
{
    public class Program
    {
        ...
        {
            using (AdultsDBContext adultsDBContext = new AdultsDBContext())
            {
                if(!adultsDBContext.Adults.Any())
                {
                    Seed(adultsDBContext);
                }    
            }
            CreateHostBuilder(args).Build().Run();
        }

        private static void Seed(AdultsDBContext adultsDBContext)
        {
